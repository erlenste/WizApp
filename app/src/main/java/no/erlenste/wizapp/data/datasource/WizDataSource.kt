...
import no.erlenste.wizapp.data.network.KtorClient
...
class WizDataSource {
    ...
    suspend fun getSpells(): List<SpellDto> {
        val response: HttpResponse = KtorClient.client.get("${baseUrl}/Spells")
    ...
    suspend fun getWizards(): List<WizardDto> {
        val response: HttpResponse = KtorClient.client.get("${baseUrl}/Wizards")
        if (response.status.value != 200) {
